...
export default function Stage2Storyboard({ projectId, timepoints, token, onApprove, onCreditsUpdate, creditsRemaining }: Stage2Props) {
  ...
  async function generateFrame(idx: number, isRegen = false) {
    ...
    try {
      ...
      if (data.allFramesDone) setAllDone(true);
    } catch (err: any) {
      setFrames(prev => prev.map((f, i) => i === idx ? { ...f, loading: false, error: err.message } : f));
    }
  }

  // Auto-generate all frames sequentially on mount
  useEffect(() => {
  ...
  const allFramesHaveImages = frames.every(f => f.imageUrl !== null);
  ...
  return (
    <div className="flex flex-col gap-5">
      <SectionHeading
        step={2}
        title="Storyboard"
        subtitle={generating ? `Generating scenes… ${frames.filter(f => f.imageUrl).length}/${timepoints.length}` : "Review and regenerate any scene (5 credits each)"}
      />
